...
const { Contract, ledger, pureCircuits, State , PublicState} = contractModule;
...
export interface DeployedBBoardAPI {
  ...
  setPrivateWeb: (privateWeb: string, newMessage: string) => Promise<void>;
}
...
export class BBoardAPI implements DeployedBBoardAPI {
    ...
    this.state$ = combineLatest(
      ...
      (ledgerState, privateState) => {
        ...
        return {
          ...
          description: ledgerState.description.value,
          sequence: ledgerState.sequence,
          isPublic: ledgerState.isPublic,  
          privateWeb: ledgerState.privateWeb.value,
  ...
  /**
   * Attempts to update the title of the current post on the bulletin board.
   *
   * @param title The new title to set.
   *
   * @remarks
   * This method can fail during local circuit execution if the bulletin board is currently vacant,
   * or if the current user is not the owner of the posted message.
   */
  async updateTitle(title: string): Promise<void> {
    this.logger?.info(`updatingTitle: ${title}`);

    const txData = await this.deployedContract.callTx.updateTitle(title);
  ...
  /**
   * Attempts to update the description of the current post on the bulletin board.
   *
  ...
  async updateDescription(description: string): Promise<void> {
    this.logger?.info(`updatingDescription: ${description}`);

    const txData = await this.deployedContract.callTx.updateDescription(description);

    this.logger?.trace({
      transactionAdded: {
        circuit: 'updateDescription',
        txHash: txData.public.txHash,
        blockHeight: txData.public.blockHeight,
      },
    });
  ...
  /**
 * Attempts to set private web content and update the message.
 *
 * @param privateWeb The private web content to set.
 * @param newMessage The new message content to set.
 */
  async setPrivateWeb(privateWeb: string, newMessage: string): Promise<void> {
    this.logger?.info(`settingPrivateWeb with message: ${newMessage}`);
    const txData = await this.deployedContract.callTx.setPrivateWeb(privateWeb, newMessage);
    this.logger?.trace({
      transactionAdded: {
        circuit: 'setPrivateWeb',
        txHash: txData.public.txHash,
        blockHeight: txData.public.blockHeight,
      },
    });
